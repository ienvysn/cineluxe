...
const BookingPage = () => {
  ...
  const getSeatPrice = (row) => {
    const basePrice = row === 'A' || row === 'B' ? pricing.frontRow : pricing.normal;
    if (isDiscountDay()) {
  ...
  const toggleSeat = (seatId) => {
    if (showtime.bookedSeats.includes(seatId) || showtime.heldSeats.includes(seatId)) return;
        ...
        <div className="container mx-auto px-4 h-20 flex items-center justify-between">
          <div className="flex items-center gap-6">
            ...
            <div className="hidden md:block">
              <h2 className="text-lg font-bold text-white leading-tight font-display">{movie.title}</h2>
              <div className="flex items-center gap-3 text-[10px] uppercase tracking-widest text-muted-foreground font-bold mt-1">
                 <span className="flex items-center gap-1"><Monitor className="w-3 h-3 text-primary" /> {screen.name}</span>
                 <span className="w-1 h-1 rounded-full bg-white/20" />
                 <span className="flex items-center gap-1 font-display italic text-primary">{showtime.time}</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
             <div className="text-right hidden sm:block">
                ...
                <p className="text-xl font-display font-bold text-primary italic">NPR {totalAmount.toLocaleString()}</p>
             </div>
             <Button
                variant="gold"
                size="lg"
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-8 flex flex-col items-center">
             ...
             <div className="w-full max-w-2xl mb-20 relative">
                ...
                <p className="text-center text-[10px] uppercase tracking-[8px] text-muted-foreground mt-4 font-bold">Screen This Way</p>
             </div>
                ...
                <div className="flex flex-col gap-3 min-w-fit px-4">
                   {rows.map((row) => (
                     <div key={row} className="flex items-center gap-3">
                        <span className="w-6 text-[10px] font-bold text-muted-foreground uppercase text-center">{row}</span>
             ...
             <div className="flex flex-wrap justify-center gap-8 mt-12 p-6 rounded-3xl bg-white/5 border border-white/5 backdrop-blur-sm">
                <div className="flex items-center gap-3">
                   ...
                   <span className="text-[10px] uppercase tracking-widest font-bold text-muted-foreground">Available</span>
                </div>
                <div className="flex items-center gap-3">
                   <div className="w-6 h-6 rounded-lg bg-white/[0.03] border border-white/5 flex items-center justify-center text-white/20">
                      <Armchair className="w-3 h-3" />
                   </div>
                   <span className="text-[10px] uppercase tracking-widest font-bold text-muted-foreground">Occupied</span>
                </div>
                <div className="flex items-center gap-3">
                   <div className="w-6 h-6 rounded-lg bg-primary shadow-lg shadow-primary/20" />
                   <span className="text-[10px] uppercase tracking-widest font-bold text-muted-foreground">Selected</span>
                </div>
             </div>
          </div>
          ...
          <div className="lg:col-span-4 space-y-6">
             <div className="glass-card p-8 border-white/10 relative overflow-hidden group">
                ...
                <div className="space-y-4 mb-8">
                   <div className="flex items-center gap-4 text-sm text-muted-foreground">
                      ...
                      <span>{format(new Date(showtime.date), 'EEEE, MMM dd, yyyy')}</span>
                   </div>
                   <div className="flex items-center gap-4 text-sm text-muted-foreground">
                      <Clock className="w-4 h-4 text-primary" />
                      <span className="font-display font-bold text-white italic">{showtime.time}</span>
                   </div>
                   <div className="flex items-center gap-4 text-sm text-muted-foreground">
                ...
                <div className="space-y-3 pt-6 border-t border-white/5">
                   <div className="flex justify-between items-center">
                      <span className="text-xs text-muted-foreground font-medium uppercase tracking-widest">Selected Seats</span>
                   ...
                   {isDiscountDay() && (
                      <div className="flex justify-between items-center text-success">
                         <span className="text-[10px] font-bold uppercase tracking-widest">Weekday Offer</span>
                         <Badge variant="success" className="text-[9px] px-2">-{pricing.discountPercent}%</Badge>
                      </div>
                   )}
                </div>
             </div>
             ...
             <div className="glass-card p-8 bg-primary/5 border-primary/20 shadow-2xl shadow-primary/5">
                <div className="flex items-center gap-3 mb-6 text-primary">
                   ...
                   <span className="text-[10px] uppercase tracking-[4px] font-black">Summary</span>
                </div>

                <div className="space-y-4 mb-8">
                   {selectedSeats.map((seatId) => (
                      <div key={seatId} className="flex justify-between items-center text-sm font-light">
                         <span className="text-muted-foreground">Seat {seatId} ({seatId.charAt(0) === 'A' || seatId.charAt(0) === 'B' ? 'Front' : 'Standard'})</span>
                         <span className="text-white font-medium">NPR {getSeatPrice(seatId.charAt(0))}</span>
                      </div>
                   ))}
                ...
                <div className="pt-6 border-t border-primary/20 flex justify-between items-end">
                   <div>
                      <p className="text-[10px] uppercase font-bold text-primary tracking-widest mb-1">Total Payable</p>
             ...
             <div className="flex items-start gap-3 p-4 bg-blue-500/5 border border-blue-500/10 rounded-2xl">
                ...
                <p className="text-[10px] text-blue-300/80 leading-relaxed font-light italic">
                   Your selection is temporarily held. Complete payment within 10 minutes to secure your tickets.
                </p>
             </div>
          </div>
